...
const Hero: React.FC<Props> = ({ content }) => {
  return (
    <div className="relative h-[85vh] w-full flex items-center justify-center bg-[#f9f9f9] overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center select-none pointer-events-none px-4">
        <h2 className="text-[12vw] font-serif-elegant font-light text-slate-200 uppercase tracking-[0.2em] opacity-30">
          Centralake
        </h2>
      </div>
      ...
      {/* Aesthetic Side Highlight (Like the teal bar in screenshot, but blue) */}
      <div className="absolute right-0 top-1/4 bottom-1/4 w-2 bg-[#0066CC]/20 hidden md:block"></div>
    </div>
  );
};
